...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            y = self.f_act(np.dot(x_data,self.W) + self.b)
            return y
...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [5,5]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.5,-0.5]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-1,0.8]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [3,5]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = [-6]         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193249     # Twój numer indeksu, np. 102247
